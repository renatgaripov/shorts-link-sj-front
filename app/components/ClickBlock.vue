    ...
    <!-- <div class="w-full bg-white rounded-4xl px-3 py-5 flex items-center justify-betwen gap-5 mb-2.5"> -->
      <div class="flex items-center justify-center bg-white rounded-l-2xl p-5">
        ...
        <div class="flex flex-col justify-between gap-1 pr-5 flex-1 bg-white py-4">
            ...
            <div class="text-xs text-[#3A3D44] bg-[#F1F4F9] px-2 py-1 rounded-md w-fit">{{ dayjs.utc(link.created_at).local().format('DD.MM.YY HH:mm:ss') || '' }}</div>
        </div>
        <div class="px-6 bg-white relative flex items-center justify-between h-full py-5 text-base text-[#3176FF] border-x-offset">
            <span class="line-clamp-1 break-all">{{ shortLink }}</span>
            <Icon name="fluent:document-copy-16-filled" class="text-2xl ml-8 flex-shrink-0 cursor-pointer" @click="copyLink" />
        </div>
        <div class="px-11 bg-white relative flex items-center justify-center h-full py-5 text-2xl text-[#3A3D44] border-r-offset">{{ link.clicks }} {{ rightWords(link.clicks as number) }}</div>
        <div class="px-6 flex items-center justify-center bg-white rounded-r-2xl">
            <div class="w-11 h-11 bg-[#F1F4F9] rounded-xl flex items-center justify-center cursor-pointer hover:opacity-50">
                <Icon name="mage:trash-fill" class="text-2xl text-[#3A3D4499] cursor-pointer" @click="$emit('deleteLink', link.id)" />
            </div>
        </div>
    <!-- </div> -->
</template>
